...
async fn init_schnorr(pubkey: &G1, commitment: &G1) -> Result<SchnorrChallenge> {
    let chacha = ChachaMiddleware::new().unwrap();
    ...
    .unwrap();
    let digest = chacha.encrypt(&body);
    let client = reqwest::Client::new();
    let resp = client
        .post(&format!("{}/chacha/protocols/sis/init", get_server("adam_b")))
        .body(digest)
        .send()
        .await?;
    ...
    let response = serde_json::from_str(&chacha.decrypt(&response_cipher).unwrap()).unwrap();
    Ok(response)
}
...
async fn prove_schnorr(session_token: String, proof: Fr) -> Result<()> {
    ...
    let client = reqwest::Client::new();
    let resp = client
        .post(&format!("{}/chacha/protocols/sis/verify", get_server("adam_b")))
        .body(digest)
        .send()
        .await?;
    ...
    let response_cipher = resp.text().await?;
    let resp: std::collections::HashMap<String, bool> = serde_json::from_str(&chacha.decrypt(&response_cipher).unwrap()).unwrap();
